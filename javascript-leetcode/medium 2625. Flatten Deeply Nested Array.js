var flat = function (arr, n) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (!isArray) answer.push(arr[i])
      ...
      // if array if n === 0 and not do flat , then push
      if (n === 0) {
        answer.push(arr[i])
      }
      // if array if n !==0 flat
      else {      
        answer.push(...flat(arr[i], n - 1))

        // 使用 concat => Time Limit Exceeded
        // answer = answer.concat(flat(arr[i], n - 1))
      }
    }
